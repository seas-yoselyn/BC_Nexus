...
def main(
    ...
    input_csv_templates = clewsb_config['clews_datapackage_template']
...
### Run the Prerequisites to update params
    cmd_schema_params_update = (  # developed CLI cmds to include multiple scripts
        ...
        "bccm update_profiling_params && " # Creates 'input_CF_csv_file' , 'input_SDP_csv_file'
        "bccm clews update_yearly_params" # Updates yearly params for new techs inside 'input_csv_case_dir'
    )
    # Run the command
    subprocess.run(cmd_schema_params_update, shell=True, text=True)
...
### UPDATING MODEL(CASE) STRUCTURE AND RUNNING MODELS
    ...
    for case_name in cases:
        ...
        output_CF_csv_file = Path(f'data/clews_data/Model_{case_name}/inputs_csv/CapacityFactor.csv') # config['FILES']['capacity_factor_file']
        
        # Updating capacity factor (averaging the values)
        CF_data=clews_builder.CFandSDP(input_CF_csv_file, 
                            representative_days, 
                            hour_grouping, 
                            output_CF_csv_file, 
                            operation='mean')
        ...
        output_SDP_csv_file =  Path(f'data/clews_data/Model_{case_name}/inputs_csv/SpecifiedDemandProfile.csv') #config['FILES']['specified_demand_profile_file']
        
        SDP_data=clews_builder.CFandSDP(input_SDP_csv_file, 
                               representative_days, 
                               hour_grouping, 
                               output_SDP_csv_file, 
                               operation='sum')
...
### Running model
        otoole_results= Path(f'results/clews/Model_{case_name}')
