...
def update_clews_builder_config(
    ...
    config=utils.load_config(clews_builder_config)


    wind_csv_file_path = config['FILES']['DATA']['wind_file']
    wind_future_pkl_file_path = config['FILES']['DATA']['wind_future_file']
    solar_csv_file_path = config['FILES']['DATA']['solar_file']
    solar_future_pkl_file_path = config['FILES']['DATA']['solar_future_file']
    tpp_csv_file_path = config['FILES']['DATA']['tpp_file']
    hydro_generation_csv_file_path = config['FILES']['DATA']['hydro_generation_file']
    hydro_resevoir_csv_file_path = config['FILES']['DATA']['hydro_reservoir_file']
    hydro_reservoir_ts_file_path = config['FILES']['DATA']['data_8760']['CF_hydro_reservoir']

    # Define cascade groups from the YAML file
    cascade_group = config['HYDRO_GENERATION']
    ...
    new_pwrwnd_structure = clews_builder.create_structure(wind_df, 'PWRWNDB')
    new_pwrwnd_future_structure = clews_builder.create_structure(wind_df, 'PWRWNDB', start_index=len(wind_df), future_df=wind_future_df)
    new_pwrsol_structure = clews_builder.create_structure(solar_df, 'PWRSOLB')
    new_pwrsol_future_structure = clews_builder.create_structure(solar_df, 'PWRSOLB', start_index=len(solar_df), future_df=solar_future_df)
    new_tpp_bio_structure = clews_builder.create_structure_tpp(tpp_df, 'PWRBIOB')
    new_tpp_ngs_structure = clews_builder.create_structure_tpp(tpp_df, 'PWRNGSB')
    new_inflow_structure = clews_builder.create_structure_inflow(hydro_generation_df, hydro_resevoir_df, hydro_reservoir_ts_df, cascade_group, 'INFLOW')
    new_hydro_structure = clews_builder.create_structure_hydro(hydro_generation_df, cascade_group, 'PWRHYDB')

    merged_pwrsol_structure = {**new_pwrsol_structure, **new_pwrsol_future_structure}
    merged_pwrwnd_structure = {**new_pwrwnd_structure, **new_pwrwnd_future_structure}

    # Update the TECHNOLOGIES section
    if 'TECHNOLOGIES' not in config:
        config['TECHNOLOGIES'] = {}

    config['TECHNOLOGIES']['PWRWND'] = merged_pwrwnd_structure
    config['TECHNOLOGIES']['PWRSOL'] = merged_pwrsol_structure
    config['TECHNOLOGIES']['PWRBIO'] = new_tpp_bio_structure
    config['TECHNOLOGIES']['PWRNGS'] = new_tpp_ngs_structure
    config['TECHNOLOGIES']['INFLOW'] = new_inflow_structure
    config['TECHNOLOGIES']['PWRHYD'] = new_hydro_structure

    # Save the updated YAML back to the same file
    with open(clews_builder_config, 'w') as file:
        yaml.dump(config, file, default_flow_style=False)

    print(f"Updated YAML file saved at: {clews_builder_config}")
    
    return (new_pwrwnd_structure,new_pwrwnd_future_structure,new_pwrsol_structure,new_pwrsol_future_structure,new_inflow_structure,new_hydro_structure)
...
def update_specified_demand_profile(
    ...
    pkl_file_path = config['FILES']['DATA']['data_8760']['SDP']
    df = pd.read_pickle(pkl_file_path)

    # Filter the DataFrame to remove February 29, 2020
    df_filtered = df[~((df.index >= '2020-02-29') & (df.index < '2020-03-01'))].copy()
