...
def generate_line_plot(df, filename_mapping, plots_directory):
    ...
    utils.print_update(level=3,
                       message="Creating annual total emission plot...")
    fig = go.Figure()
...
def generate_stacked_bar_chart(technologies,all_years, yearly_summed_values, filename_mapping, custom_colors, legend_labels, plots_directory):
    ...
    utils.print_update(level=3,
                       message="Creating sectoral annual emission plot...")
    traces = []
    for tech in technologies:
...
def make_plots(model_results_directory, filenames, filenames_mapping, technologies, custom_colors, legend_labels, plots_directory):
    ...
    for filename in filenames:
        ...
        utils.print_update(level=3,
                             message=f"Plots generated successfully and saved as HTML files to the output directory: {plots_directory}")
