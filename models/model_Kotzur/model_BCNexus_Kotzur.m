...
set TIMESLICEofLDC{ldc in DAYSCRO} within DAYTYPE := {ld in DAYTYPE : Conversionldc[ld,ldc] = 1};
...
var StorageLevelDayTypeFinish{r in REGION, s in STORAGE, ld in DAYTYPE, y in YEAR};
var StorageLevelChronoDayStart{r in REGION, s in STORAGE, ldc in DAYSCRO, y in YEAR} >=0;
...
#	else StorageLevelYearStart[r,s,y-1] + sum{l in TIMESLICE} (sum{t in TECHNOLOGY, m in MODE_OF_OPERATION:TechnologyToStorage[r,t,s,m]>0} (RateOfActivity[r,l,t,m,y-1] * #TechnologyToStorage[r,t,s,m]) - (sum{t in TECHNOLOGY, m in MODE_OF_OPERATION:TechnologyFromStorage[r,t,s,m]>0} RateOfActivity[r,l,t,m,y-1] * TechnologyFromStorage[r,t,s,m])) * #YearSplit[l,y-1]
#	= StorageLevelYearStart[r,s,y];

s.t. S2_StorageLevelDayTypeFinish{r in REGION, s in STORAGE, ld in DAYTYPE, y in YEAR}: sum{l in TIMESLICEofDAYTYPE[ld], t in TECHNOLOGY, m in MODE_OF_OPERATION:TechnologyToStorage[r,t,s,m]>0} (RateOfActivity[r,l,t,m,y] * TechnologyToStorage[r,t,s,m]* DaySplit[l,y]) - sum{l in TIMESLICEofDAYTYPE[ld], t in TECHNOLOGY, m in MODE_OF_OPERATION:TechnologyFromStorage[r,t,s,m]>0} (RateOfActivity[r,l,t,m,y] * TechnologyFromStorage[r,t,s,m]* DaySplit[l,y])
	= StorageLevelDayTypeFinish[r,s,ld,y];

s.t. S1_StorageLevelYearStart{r in REGION, s in STORAGE, y in YEAR}: if y = min{yy in YEAR} min(yy) then StorageLevelStart[r,s]
	else StorageLevelYearStart[r,s,y-1] + sum{ldc in DAYSCRO} (sum{ld in TIMESLICEofLDC[ldc]}(StorageLevelDayTypeFinish[r,s,ld,y-1]))
	= StorageLevelYearStart[r,s,y];

s.t. S3_StorageLevelChronoDayStart{r in REGION, s in STORAGE, ldc in DAYSCRO, y in YEAR}: if ldc = min{ldcldc in DAYSCRO} min(ldcldc) then StorageLevelYearStart[r,s,y] 
	else StorageLevelChronoDayStart[r,s,ldc-1,y] + sum{ld in TIMESLICEofLDC[ldc-1]}(StorageLevelDayTypeFinish[r,s,ld,y]) 
	= StorageLevelChronoDayStart[r,s,ldc,y];

s.t. SC4_StorageRefilling{s in STORAGE, r in REGION}: sum{y in YEAR, ldc in DAYSCRO} (sum{ld in TIMESLICEofLDC[ldc]}(StorageLevelDayTypeFinish[r,s,ld,y])) = 0;
#
##########                Storage Constraints                                #############
#
s.t. SC1_LowerLimit{r in REGION, s in STORAGE, ldc in DAYSCRO, y in YEAR}:  MinStorageCharge[r,s,y]*(sum{yy in YEAR: y-yy < OperationalLifeStorage[r,s] && y-yy>=0} NewStorageCapacity[r,s,yy]+ResidualStorageCapacity[r,s,y]) <= StorageLevelChronoDayStart[r,s,ldc,y];

s.t. SC2_UpperLimit{r in REGION, s in STORAGE, ldc in DAYSCRO, y in YEAR}: StorageLevelChronoDayStart[r,s,ldc,y] <= (sum{yy in YEAR: y-yy < OperationalLifeStorage[r,s] && y-yy>=0} NewStorageCapacity[r,s,yy]+ResidualStorageCapacity[r,s,y]);

#s.t. SC3_StorageMaxUpperLimit{s in STORAGE, y in YEAR, r in REGION}:  (sum{yy in YEAR: y-yy < OperationalLifeStorage[r,s] && y-yy>=0} #NewStorageCapacity[r,s,yy]+ResidualStorageCapacity[r,s,y]) <= StorageMaxCapacity[r,s,y];
#
#########                Storage Investments                                #############
#
...
#
s.t. TCC1_TotalAnnualMaxCapacityConstraint{r in REGION, t in TECHNOLOGY, y in YEAR: TotalAnnualMaxCapacity[r,t,y] <> -1}: ((sum{yy in YEAR: y-yy < OperationalLife[r,t] && y-yy>=0} NewCapacity[r,t,yy])+ ResidualCapacity[r,t,y]) <= TotalAnnualMaxCapacity[r,t,y];
...
#
s.t. AAC2_TotalAnnualTechnologyActivityUpperLimit{r in REGION, t in TECHNOLOGY, y in YEAR: TotalTechnologyAnnualActivityUpperLimit[r,t,y] <> -1}: sum{l in TIMESLICE, m in MODEperTECHNOLOGY[t]} RateOfActivity[r,l,t,m,y]*YearSplit[l,y] <= TotalTechnologyAnnualActivityUpperLimit[r,t,y] ;
...
#
s.t. TAC2_TotalModelHorizonTechnologyActivityUpperLimit{r in REGION, t in TECHNOLOGY: TotalTechnologyModelPeriodActivityUpperLimit[r,t] <> -1}: sum{l in TIMESLICE, m in MODEperTECHNOLOGY[t], y in YEAR} RateOfActivity[r,l,t,m,y]*YearSplit[l,y] <= TotalTechnologyModelPeriodActivityUpperLimit[r,t] ;
...
#s.t. E4_EmissionsPenaltyByTechnology{r in REGION, t in TECHNOLOGY, y in YEAR}: sum{e in EMISSION, l in TIMESLICE, m in MODEperTECHNOLOGY[t]} EmissionActivityRatio[r,t,e,m,y]*RateOfActivity[r,l,t,m,y]*YearSplit[l,y]*EmissionsPenalty[r,e,y] = AnnualTechnologyEmissionsPenalty[r,t,y];
s.t. E6_EmissionsAccounting1{r in REGION, e in EMISSION, y in YEAR}: sum{l in TIMESLICE, t in TECHNOLOGY, m in MODEperTECHNOLOGY[t]: EmissionActivityRatio[r,t,e,m,y]<>0} EmissionActivityRatio[r,t,e,m,y]*RateOfActivity[r,l,t,m,y]*YearSplit[l,y]+ AnnualExogenousEmission[r,e,y] = AnnualEmissions[r,e,y] ;
...
#set MODExTECHNOLOGYperSTORAGEfrom{STORAGE} within MODE_OF_OPERATION cross TECHNOLOGY;
#
table tout {r in REGION, t in TECHNOLOGY, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/AccumulatedNewCapacity.csv" : r, t, y, AccumulatedNewCapacity[r, t, y];
table tout {r in REGION, e in EMISSION, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/AnnualEmissions.csv" : r, e, y, AnnualEmissions[r, e, y];
table tout {r in REGION, t in TECHNOLOGY, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/AnnualFixedOperatingCost.csv" : r, t, y, AnnualFixedOperatingCost[r, t, y];
table tout {r in REGION, t in TECHNOLOGY, e in EMISSION, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/AnnualTechnologyEmission.csv" : r, t, e, y, AnnualTechnologyEmission[r, t, e, y];
table tout {r in REGION, t in TECHNOLOGY, e in EMISSION, m in MODEperTECHNOLOGY[t], y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/AnnualTechnologyEmissionByMode.csv" : r, t, e, m, y, AnnualTechnologyEmissionByMode[r, t, e, m, y];
table tout {r in REGION, t in TECHNOLOGY, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/AnnualVariableOperatingCost.csv" : r, t, y, AnnualVariableOperatingCost[r, t, y];
table tout {r in REGION, t in TECHNOLOGY, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/CapitalInvestment.csv" : r, t, y, CapitalInvestment[r, t, y];
table tout {r in REGION, l in TIMESLICE, f in FUEL, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/Demand.csv" : r, l, f, y, Demand[r, l, f, y];
table tout {r in REGION, t in TECHNOLOGY, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/DiscountedSalvageValue.csv" : r, t, y, DiscountedSalvageValue[r, t, y];
table tout {r in REGION, t in TECHNOLOGY, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/DiscountedTechnologyEmissionsPenalty.csv" : r, t, y, DiscountedTechnologyEmissionsPenalty[r, t, y];
table tout {r in REGION, t in TECHNOLOGY, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/NewCapacity.csv" : r, t, y, NewCapacity[r, t, y];
table tout {r in REGION, s in STORAGE, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/NewStorageCapacity.csv" : r, s, y, NewStorageCapacity[r, s, y];
table tout {r in REGION, t in TECHNOLOGY, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/NumberOfNewTechnologyUnits.csv" : r, t, y, NumberOfNewTechnologyUnits[r, t, y];
table tout {r in REGION, t in TECHNOLOGY, f in FUEL, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/ProductionByTechnologyAnnual.csv" : r, t, f, y, ProductionByTechnologyAnnual[r, t, f, y];
table tout {r in REGION, l in TIMESLICE, t in TECHNOLOGY, m in MODEperTECHNOLOGY[t], y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/RateOfActivity.csv" : r, l, t, m, y, RateOfActivity[r, l, t, m, y];
table tout {r in REGION, t in TECHNOLOGY, l in TIMESLICE, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/RateOfTotalActivity.csv" : r, t, l, y, RateOfTotalActivity[r, t, l, y];
table tout {r in REGION, t in TECHNOLOGY, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/SalvageValue.csv" : r, t, y, SalvageValue[r, t, y];
table tout {r in REGION, s in STORAGE, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/SalvageValueStorage.csv" : r, s, y, SalvageValueStorage[r, s, y];
table tout {r in REGION, t in TECHNOLOGY, m in MODEperTECHNOLOGY[t], y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/TotalAnnualTechnologyActivityByMode.csv" : r, t, m, y, TotalAnnualTechnologyActivityByMode[r, t, m, y];
table tout {r in REGION, t in TECHNOLOGY, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/TotalCapacityAnnual.csv" : r, t, y, TotalCapacityAnnual[r, t, y];
table tout {r in REGION, t in TECHNOLOGY, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/TotalTechnologyAnnualActivity.csv" : r, t, y, TotalTechnologyAnnualActivity[r, t, y];
table tout {r in REGION, t in TECHNOLOGY} OUT "CSV" "Model_Kotzur/res/csv/TotalTechnologyModelPeriodActivity.csv" : r, t, TotalTechnologyModelPeriodActivity[r, t];
table tout {r in REGION, rr in REGION, l in TIMESLICE, f in FUEL, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/Trade.csv" : r, rr, l, f, y, Trade[r, rr, l, f, y];
table tout {r in REGION, s in STORAGE, ld in DAYTYPE, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/StorageLevelDayTypeFinish.csv" : r, s, ld, y, StorageLevelDayTypeFinish[r, s, ld, y];
table tout {r in REGION, s in STORAGE, ldc in DAYSCRO, y in YEAR} OUT "CSV" "Model_Kotzur/res/csv/StorageLevelChronoDayStart.csv" : r, s, ldc, y, StorageLevelChronoDayStart[r, s, ldc, y];
table tout {r in REGION, s in STORAGE, l in TIMESLICE, y in YEAR : sum {t in TECHNOLOGY, m in MODE_OF_OPERATION : TechnologyToStorage[r, t, s, m] > 0} (RateOfActivity[r, l, t, m, y] * TechnologyToStorage[r, t, s, m] * DaySplit[l, y]) - sum {t in TECHNOLOGY, m in MODE_OF_OPERATION : TechnologyFromStorage[r, t, s, m] > 0} (RateOfActivity[r, l, t, m, y] * TechnologyFromStorage[r, t, s, m] * DaySplit[l, y])} OUT "CSV" "Model_Kotzur/res/csv/StorageLevelTSResults.csv" : r, s, l, y, sum {t in TECHNOLOGY, m in MODE_OF_OPERATION : TechnologyToStorage[r, t, s, m] > 0} (RateOfActivity[r, l, t, m, y] * TechnologyToStorage[r, t, s, m] * DaySplit[l, y]) - sum {t in TECHNOLOGY, m in MODE_OF_OPERATION : TechnologyFromStorage[r, t, s, m] > 0} (RateOfActivity[r, l, t, m, y] * TechnologyFromStorage[r, t, s, m] * DaySplit[l, y]) ~VALUE;
table tout {r in REGION, s in STORAGE : StorageLevelStart[r, s] > 0} OUT "CSV" "Model_Kotzur/res/csv/StorageLevelStartResults.csv" : r, s, StorageLevelStart[r, s] ~VALUE;
#
end;
