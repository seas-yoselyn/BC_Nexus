...
def collect_files(source_dir, destination_dir):
    ...
    target_folders = ['Constraints - Activity', 'Demand', 'Performance', 'Sets/Sets- Not in use', 'Sets/clewsy_output', 'VARIABLES',
                      'Constraints - Capacity', 'Emissions', 'RE Gen Target', 'Storage',
                      'Constraints - Investment', 'Global Parameters', 'Reserve Margin', 'Technology Cost', 'Additional Parameters']
...
def collect_csv_files(source_dir, destination_dir):
    ...
    for item in os.listdir(source_dir):
        ...
        if os.path.isfile(item_path) and item.lower().endswith(".csv"):
            ...
            if item_path == "user_inputs/Sets/clewsy_output/COMMODITY.csv":
